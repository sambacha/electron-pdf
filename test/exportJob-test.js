...
test("setSessionCookie_multiple", (t) => {
  const cs = setupCookieStub();
  job._setSessionCookies(
    "C1=foo;JSESSIONID=bar;C2=baz",
    "http://host",
    cs.stub
  );
...
test("_getHeaders all possible headers", (t) => {
  ...
  const expected = ["pragma: no-cache", "H1: V1", "H2: V2"];
...
test.cb(
  `_waitForPage with --noprint adds no wait and invokes generate function immediately`,
  (t) => {
    args.noprint = true;
