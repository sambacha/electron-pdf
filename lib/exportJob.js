...
const HUNG_WINDOW_CLEANUP_INTERVAL =
  process.env.ELECTRONPDF_WINDOW_CLEANUP_INTERVAL || 1000 * 30; /* seconds */
...
class ExportJob extends EventEmitter {
  ...
  constructor(input, output, args, options) {
    ...
    const validOrientation =
      _.size(this.options.orientations) === _.size(this.input);
  ...
  _initializeWindowForResource(landscape) {
    ...
    const dim = WindowTailor.setWindowDimensions(
      this.window,
      this.args.pageSize,
      landscape
    );
  ...
  _getBrowserConfiguration(args) {
    const pageDim = WindowTailor.getPageDimensions(
      args.pageSize,
      args.landscape
    );
    ...
    try {
      ...
      errorLogger(
        "Invalid browserConfig provided, using defaults. Value:",
        args.browserConfig,
        "\nError:",
        e
      );
  ...
  _getHeaders() {
    ...
    if (this.args.requestHeaders) {
      _.each(JSON.parse(this.args.requestHeaders), (v, k) =>
        headers.push(`${k}: ${v}`)
      );
    }
    return headers;
  ...
  _load(win) {
    ...
    if (this.args.noprint) {
      ...
      this._renderAndCollectOutput(
        win,
        (context, outputDoneFn, observerContext) => {
  ...
  _renderAndCollectOutput(win, generateFunc) {
    ...
    this.input.forEach((uriPath, inputIndex) => {
      ...
      windowEvents.push((outputDoneFn) =>
        this._captureWindow(context, generateFunc, outputDoneFn)
      );
    });
    ...
    async.series(windowEvents, (err, results) => {
       ...
       */
      this.emit(`${RENDER_EVENT_PREFIX}complete`, {
        results: results,
        error: err,
      });
  ...
  _captureWindow(context, generateFn, outputDoneFn) {
    ...
    const waitFunction = this._waitForPage.bind(
      this,
      window,
      generateFunction,
      this.args.outputWait
    );
  ...
  _waitForBrowserEvent(waitForJSEvent, window, generateFunction) {
    const eventName =
      _.size(waitForJSEvent) > 0 ? waitForJSEvent : "view-ready";
  ...
  _executeJSListener(eventName, ipcListener, generateFunction, window) {
    ...
    const f = this._cancelReadyEvent.bind(
      this,
      eventName,
      ipcListener,
      generateFunction
    ...
    const maxWait =
      this.args.outputWait > 0 ? this.args.outputWait : MAX_READY_EVENT_WAIT;
  ...
  _getTargetFile(context) {
    ...
    if (_.size(this.input) > 1) {
      return (
        this.output.substring(0, extIndex) +
        `_${zeroBasedIndex + 1}${actualExt}`
      );
    }
