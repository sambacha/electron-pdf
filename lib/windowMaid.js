...
const HUNG_WINDOW_THRESHOLD =
  process.env.ELECTRONPDF_WINDOW_LIFE_THRESHOLD || 1000 * 60 * 5; /* minutes */
  ...
  cleanupHungWindows(threshold) {
    ...
    _.forEach(hungWindows, (e) => {
      ...
      if (destroyable) {
        ...
        errorLogger(
          "a window was left in the cache that was already destroyed, do proper cleanup"
        );
        delete windowCache[e.id];
      }
    });
